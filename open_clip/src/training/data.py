...
class CsvDataset(Dataset):
    ...
    def pad_image(self, image:Image):
        height, width = image.size
        ratio = 224 / max(height, width)
        new_height = int(height * ratio)
        new_width = int(width * ratio)
        resized_image = image.resize((new_height,new_width), Image.BICUBIC)
        ...
        padded_image = ImageOps.expand(resized_image, border=(top_pad, left_pad , bottom_pad, right_pad ), fill=(0, 0, 0))

        return padded_image

    def __getitem__(self, idx):
        # images = self.transforms(self.pad_image(Image.open(str(self.images[idx]))))
        images = self.transforms((Image.open(str(self.images[idx]))))
        texts = self.tokenize([str(self.captions[idx])])[0]
        return images, texts

class CsvDataset_ids(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, sep="\t", tokenizer=None):
        logging.debug(f'Loading csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)
        ...
        self.images = df[img_key].tolist()
        self.captions = df[caption_key].tolist()
        self.classes = df["class"].tolist() # for sample
        self.classes_uniq = list(set(self.classes))
        self.transforms = transforms
        logging.debug('Done loading data.')

        self.tokenize = tokenizer

        self.ids = self.map_ids()

    def map_ids(self):
        ids = []
        for cls in self.classes:
            ids.append(self.classes_uniq.index(cls))
        return ids
    ...
    def __getitem__(self, idx):
        ...
        return images, texts, self.ids[idx]
